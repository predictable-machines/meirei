...
partial def elabSimpleFold
  ...
  let loopVarInfo : VarInfo :=
    { name := loopVarName, type := (MeireiType.named `Int), currentVersion := 0 }
  foldCtx := { foldCtx with vars := foldCtx.vars.insert loopVarName loopVarInfo }
  set foldCtx
